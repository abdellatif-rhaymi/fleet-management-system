...
def generate_route_map(coords_file):
    ...
    try:
        ...
        vehicle_end = coords[-1]  # Dernier point comme point d'arrivée
        ...
        vehicles = [
            ors.optimization.Vehicle(
                ...
                profile='driving-car', 
                start=vehicle_start, 
                end=vehicle_end,  # Dernier point comme point final
                capacity=[10]
            )
        ]
        ...
        for route in optimized['routes']:
            total_distance += route['distance']  # Distance en mètres
            total_duration += route['duration']  # Durée en secondes

        # Afficher la distance et le temps total
        datadetail = {
    'total_distance': total_distance / 1000,  # Convertir en kilomètres
    'total_duration': total_duration / 60    # Convertir en minutes
}
        
        url = "http://localhost:8082/s3_new_2/trajet.trajet"
        response = requests.get(url, data=datadetail)


        print(f"Distance totale : {total_distance / 1000:.2f} km")
        print(f"Temps total : {total_duration / 60:.2f} minutes")
        ...
        html_path = os.path.join(webapp_dir, 'mapof3.jsp')
        m.save(html_path)
